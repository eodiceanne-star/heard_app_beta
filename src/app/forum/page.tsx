            ...
            <form onSubmit={handleCreateThread} className="space-y-6 relative z-10">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-4 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-dusty-pink focus:border-transparent text-base resize-none"
                  rows={4}
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-full text-base font-medium transition-colors ${
                        newThread.selectedTags.includes(tag)
                          ? 'bg-dusty-pink text-black shadow-md'
              ...
              <button type="submit" className="mobile-button w-full text-lg py-4 text-black">
                Post Discussion
              </button>
            </form>
          </div>
            ...
            <div key={thread.id} className="mobile-card relative overflow-hidden">
              ...
              <div className="absolute top-0 left-0 w-12 h-12 bg-gradient-to-br from-sage to-transparent opacity-10 rounded-full -ml-6 -mt-6 z-0"></div>
                             ...
                             <div className="flex items-start space-x-4 mb-4 relative z-10">
                                   <div className="w-12 h-12 bg-gradient-to-br from-cream to-dusty-pink rounded-full flex items-center justify-center text-charcoal text-xl flex-shrink-0 shadow-md p-1 avatar-container">
                ...
                <div className="flex-1">
                  ...
                  <h3 className="text-xl font-medium text-charcoal mb-3 text-visible">{thread.title}</h3>
                  <p className="text-base text-charcoal leading-relaxed mb-4 forum-content">{thread.content}</p>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    {thread.tags.map(tag => (
                                        <span key={tag} className="px-3 py-1 bg-sage bg-opacity-20 text-black text-sm rounded-full font-medium tag">
                    {tag}
                  </span>
                    ))}
                  </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-100">
                      <div className="space-y-4 mb-6 max-h-96 overflow-y-auto pr-2">
                        {thread.comments.map(comment => (
                          <div key={comment.id} className="flex space-x-4">
                                                         <div className="w-10 h-10 bg-gradient-to-br from-cream to-dusty-pink rounded-full flex items-center justify-center text-charcoal text-lg flex-shrink-0 shadow-md p-1 avatar-container">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-base text-charcoal comment-content">{comment.content}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex space-x-3">
                        <textarea
                          ...
                          rows={3}
                          className="flex-1 p-3 border border-gray-200 rounded-xl text-base focus:ring-2 focus:ring-dusty-pink focus:border-transparent resize-none"
        ...
        {showReportDialog && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-6 z-50">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
              ...
              <p className="text-base text-charcoal mb-8 leading-relaxed modal-content">
